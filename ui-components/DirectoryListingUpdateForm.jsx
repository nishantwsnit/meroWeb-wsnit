...
import { getDirectoryListing } from "./graphql/queries";
import { updateDirectoryListing } from "./graphql/mutations";
...
export default function DirectoryListingUpdateForm(props) {
  const {
    id: idProp,
    directoryListing: directoryListingModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = directoryListingRecord
      ? { ...initialValues, ...directoryListingRecord }
      : initialValues;
    setTitle(cleanValues.title);
    setDescription(cleanValues.description);
    setAddress(cleanValues.address);
    setUrl(cleanValues.url);
    setPhoneNumbers(cleanValues.phoneNumbers ?? []);
    setCurrentPhoneNumbersValue("");
    setCompanyImage(cleanValues.companyImage);
    setImages(cleanValues.images ?? []);
    setCurrentImagesValue("");
    setDirectoryId(cleanValues.directoryId);
    setErrors({});
  };
  const [directoryListingRecord, setDirectoryListingRecord] = React.useState(
    directoryListingModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getDirectoryListing.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDirectoryListing
        : directoryListingModelProp;
      setDirectoryListingRecord(record);
    };
    queryData();
  }, [idProp, directoryListingModelProp]);
  React.useEffect(resetStateValues, [directoryListingRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateDirectoryListing.replaceAll("__typename", ""),
            variables: {
              input: {
                id: directoryListingRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "DirectoryListingUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || directoryListingModelProp)}

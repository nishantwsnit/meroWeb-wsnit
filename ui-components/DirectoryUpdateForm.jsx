...
import { getDirectory } from "./graphql/queries";
import { updateDirectory } from "./graphql/mutations";
const client = generateClient();
export default function DirectoryUpdateForm(props) {
  const {
    id: idProp,
    directory: directoryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = directoryRecord
      ? { ...initialValues, ...directoryRecord }
      : initialValues;
    setTitle(cleanValues.title);
    setSubTitle(cleanValues.subTitle);
    setImageUrl(cleanValues.imageUrl);
    setIsSubDirectory(cleanValues.isSubDirectory);
    setParentDirectoryId(cleanValues.parentDirectoryId);
    setErrors({});
  };
  const [directoryRecord, setDirectoryRecord] =
    React.useState(directoryModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getDirectory.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDirectory
        : directoryModelProp;
      setDirectoryRecord(record);
    };
    queryData();
  }, [idProp, directoryModelProp]);
  React.useEffect(resetStateValues, [directoryRecord]);
  const validations = {
    title: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          isSubDirectory: isSubDirectory ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateDirectory.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "DirectoryUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || directoryModelProp)}
